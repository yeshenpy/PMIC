...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, arglist):
        ...
        self.mujoco_name = getattr(arglist, "mujoco_name", "HalfCheetah-v2")
        self.agent_conf = getattr(arglist, "agent_conf", "2x3")

        self.agent_partitions, self.mujoco_edges  = obsk.get_parts_and_edges(self.mujoco_name,
                                                                             self.agent_conf)
        ...
        if self.env_version == 2:
            self.wrapped_env = NormalizedActions(gym.make(self.mujoco_name))
        else:
